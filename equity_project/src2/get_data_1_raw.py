...
def load_data():
    """Скачиваем сырые данные, сохраняем"""
    ...
    data.to_parquet(
        project_path.as_posix() + "/data/raw/all_data.parquet", engine="pyarrow"
    )

    with open(project_path.as_posix() + "/data/raw/first_appearance_dict.json", 'w', encoding='utf-8') as file:
        json.dump(first_appearance_dict, file, ensure_ascii=False)
